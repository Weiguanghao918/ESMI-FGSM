# coding=utf-8
"""Implementation of ESMI-FGSM-ENS attack."""
...
def transform(x):
    ret = random.randint(270, 299)
    crop_image_tensor = tf.image.resize_with_crop_or_pad(x, ret, ret)
    pad_image_tensor = input_diversity(crop_image_tensor, ret, 299)
    return pad_image_tensor


def batch_grad(x, one_hot, i, max_iter, alpha, grad):
    eps = 2.0 * FLAGS.max_epsilon / 255.0
    x_neighbor = x + tf.random_uniform(x.shape, minval=- eps * FLAGS.beta, maxval=eps * FLAGS.beta)
    x_neighbor = transform(x_neighbor)
    ...
    with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
        logits_inresv2, end_points_inrestv2 = inception_resnet_v2.inception_resnet_v2(
            x_neighbor, num_classes=1001, is_training=False, reuse=tf.AUTO_REUSE)

    with slim.arg_scope(resnet_v2.resnet_arg_scope()):
        logits_resv2, end_points_resv2 = resnet_v2.resnet_v2_101(
            x_neighbor, num_classes=1001, is_training=False, reuse=tf.AUTO_REUSE)
    logit = (logits_v3 + logits_v4 + logits_inresv2 + logits_resv2) / 4
    auxlogits = (end_points_v3['AuxLogits'] + end_points_v4['AuxLogits'] + end_points_inrestv2['AuxLogits']) / 3

    cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logit, label_smoothing=0.0, weights=1.0)
    cross_entropy += tf.losses.softmax_cross_entropy(one_hot,
                                                     auxlogits,
                                                     label_smoothing=0.0,
                                                     weights=0.4)
    grad += ((tf.gradients(cross_entropy, x)[0]) / (1. * FLAGS.spatial_number))
    i = tf.add(i, 1)
...
def graph(x, y, i, x_max, x_min, grad, variance):
    ...
    pred = tf.argmax(end_points_v3['Predictions'] + end_points_v4['Predictions'] + end_points_inrestv2['Predictions'] +
                     end_points_resv2['predictions'], 1)
    ...
    _, _, _, _, _, global_grad = tf.while_loop(grad_finish, batch_grad,
                                               [x, one_hot, iter, max_iter, eps * FLAGS.beta, tf.zeros_like(new_grad)])

    variance = global_grad / (1. * FLAGS.number)
    current_grad = variance
...
def stop(x, y, i, x_max, x_min, grad, variance):
    num_iter = FLAGS.num_iter
...
def input_diversity(input_tensor, ret, resize_scale):
    rnd = tf.random_uniform((), ret, resize_scale, dtype=tf.int32)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        x_adv, _, _, _, _, _, _ = tf.while_loop(stop, graph, [x_input, y, i, x_max, x_min, tf.zeros(shape=batch_shape),
                                                              tf.zeros(shape=batch_shape)])
