# coding=utf-8
"""Implementation of ESMI-CT-FGSM attack."""
...
def batch_grad(x, one_hot, i, max_iter, alpha, grad):
    ...
    x = x +tf.random_uniform(x.shape, minval=- eps * FLAGS.beta, maxval=eps * FLAGS.beta)
    x = transform(x)
    x_neighbor_2 = 1 / 2. * x
    x_neighbor_4 = 1 / 4. * x
    x_neighbor_8 = 1 / 8. * x
    x_neighbor_16 = 1 / 16. * x
    x_res = tf.concat([x, x_neighbor_2, x_neighbor_4, x_neighbor_8, x_neighbor_16], axis=0)
    with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
        logits_v3, end_points_v3 = inception_v4.inception_v4(
            input_diversity_DIM(x_res), num_classes=1001, is_training=False, reuse=tf.AUTO_REUSE)
        cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logits_v3)
        grad += (tf.reduce_sum(
            tf.split(tf.gradients(cross_entropy, x_res)[0], 5) * tf.constant([1, 1 / 2., 1 / 4., 1 / 8., 1 / 16.])[:,
...
def graph(x, y, i, x_max, x_min, grad, variance):
    ...
    with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
        logits_v3, end_points_v3 = inception_v4.inception_v4(
            input_diversity_DIM(x_batch), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
...
def input_diversity_DIM(input_tensor):
